...
exports.getAllTours = async (filter) => {
  ...
  if (filter) {
    ...
    for (let criteria in filter) {
      ...
      } else if (criteria === "name" || criteria === "difficulty") {
        filterStr += criteria + " = '" + filter[criteria] + "'";
        i++;
      }
    }
...
exports.getTourById = async (id) => {
  ...
  let result = await request
    ...
    .query(
      `select * from ${TourSchema.schemaName} where ${TourSchema.schema.id.name} = @${TourSchema.schema.id.name}`
    );
...
exports.getTourByName = async (name) => {
  ...
  let result = await request
    .input(
      `${TourSchema.schema.name.name}`,
      TourSchema.schema.name.sqlType,
      name
    )
    .query(
...
exports.deleteTourById = async (id) => {
  ...
  let result = await request
    .input(`${TourSchema.schema.id.name}`, TourSchema.schema.id.sqlType, id)
    .query(
      `delete ${TourSchema.schemaName} where ${TourSchema.schema.id.name} = @${TourSchema.schema.id.name}`
    );
...
exports.updateTourById = async (id, updateInfo) => {
  ...
  if (
    typeof updateInfo.rating === "number" &&
    updateInfo.rating >= 0 &&
    updateInfo.rating <= 5
  ) {
...
exports.clearAll = async () => {
  ...
  let result = await dbConfig.db.pool
    .request()
    .query(`delete ${TourSchema.schemaName}`);
